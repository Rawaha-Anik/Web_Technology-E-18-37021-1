<?php
	...
	$date=$dateErr=$gender=$genderErr=$dateErr=$selectRoleErr=$selectBloodGroupErr=$role=$bloodgroup="";
	$gender="Male";
		...
		if($allOk === true){
            ...
            if($emailexist==true){
                ...
                if(empty($selectBloodGroupErr)&&empty($selectErr)&&empty($nameErr)&&empty($passwordErr)&&empty($selectErr)&&empty($confirmpasswordErr)&&empty($emailErr)&&empty($genderErr)&&empty($usernameErr)&&empty($dateErr)){  
					...
					$current_data = file_get_contents('D:\XAMPP\htdocs\FinalTerm\ProjectFinal\Controller\UserApprovalController\ApproveList.json');  
    				$array_data = json_decode($current_data, true);  
   					$array_data[] = $extra;  
    				$final_data = json_encode($array_data);  
  					if(file_put_contents('D:\XAMPP\htdocs\FinalTerm\ProjectFinal\Controller\UserApprovalController\ApproveList.json',$final_data)){
        				$message = "<h1 style='color:Black;'>File Appended Successfully</h1>";  
        				echo $message;
    				}
					$username="";
					$name="";
					...
					$datefromform="";
					$gender="";
					header('Location: ../Controller/UserApprovalController/ControllerAddUserApprovalByGet.php?Username='.$extra["Username"]);
				}
            }
		}
	} 
