...
?>
<?php
    include 'D:\XAMPP\htdocs\FinalTerm\ProjectFinal\Controller\AdminController\ControllerUpdateAdmin.php';
    $nameErr = $emailErr = $genderErr = $usernameErr = $passwordErr=$confirmpasswordErr=$dateErr="";
    $name = $email = $gender = $comment = $website = $username=$password=$confirmpassword=$errorJSON=$message=$selectBloodGroupErr="";
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        $userexist=false;
        $emailexist=false;
        $data=showAdminByEmail($_SESSION['Email']);  
        ...
        if($emailexist==true){
            ...
            if(empty($nameErr)&&empty($passwordErr)&&empty($confirmpasswordErr)&&empty($emailErr)&&empty($genderErr)&&empty($usernameErr)&&empty($dateErr)){  
                ...
                ControllerUpdateAdmin($_SESSION['Username'],$extra);       
                ...
                setcookie('BloodGroup',$_SESSION["BloodGroup"],time()+1000);
				...
				<fieldset style="width: 50%">
                        ...
                        <fieldset style="width: 50%" class="field_set" align="left">
    	    			    <legend>Blood Group:</legend> 
   		    			    <select name ="BloodGroup" value="<?php echo $_SESSION['BloodGroup'];?>">
        	    		    	<option value=""></option>
